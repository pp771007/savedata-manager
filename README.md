...
| 🔄 **匯出 / 匯入** | 把這台電腦的**所有手動備份**帶到另一台，兩種方式任選：**① 檔案** —— 打包成 JSON 檔下載 / 讀入；**② 文字字串** —— 複製成一段 base64 文字（免存檔，貼到雲端筆記 / 通訊軟體即可搬運），另一台**貼上後自動匯入**。匯出的是**備份清單**而非網頁當下的存檔；若目前存檔還沒備份，匯出時會**主動詢問要不要先備份一份一起帶走**（完全沒備份時也一樣，一鍵完成備份＋匯出）。 |
...
- 匯出 / 複製**只包含手動建立的備份**，自動備份不會被帶走。匯出的是備份清單、**不是網頁當下的存檔**——若目前存檔還沒備份，工具會先問你要不要補備份一份一起匯出。
